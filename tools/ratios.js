...
(async () => {
  ...
  if (COPY[copyKey]) {
    await page.evaluate(c => {
      ...
      window.__NF.setTextField('tiEyebrow', c.eyebrow);
      window.__NF.setTextField('tiHead', c.head);
      window.__NF.setTextField('tiBody', c.body);
  ...
  const rows = [];
  for (const r of list) {
    await page.evaluate(([w, h]) => window.__NF.setCanvasSize(w, h), [r.w, r.h]);
    await settle(page);
