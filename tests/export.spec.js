...
const { open, settle, expandAll, pickSeg, state, captureExport, setCanvasSize } = require('./helpers');
...
test.describe('export', () => {
  ...
  test('WebP export produces a non-trivial image', async ({ page }) => {
    await pickSeg(page, 'expFmt', 'webp');
    const r = await captureExport(page);
    expect(r.blob).not.toBeNull();
    expect(r.blob.type).toMatch(/webp|png/);
    expect(r.blob.size).toBeGreaterThan(5_000);
    expect(r.exporting).toBe(false);
  });
  ...
  test('a size that exceeds the single-pass budget is rendered in tiles', async ({ page }) => {
    await setCanvasSize(page, 8192, 4096);
    await settle(page);
  ...
  test('the footer reports the size that will be exported', async ({ page }) => {
    await setCanvasSize(page, 1500, 500);
    await settle(page);
  ...
  test('video export records a playable file', async ({ page }) => {
    ...
    await setCanvasSize(page, 480, 480);
    await settle(page);
