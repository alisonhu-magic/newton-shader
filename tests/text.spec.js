...
const { open, settle, expandAll, pickSeg, state, commitInput, fieldValue } = require('./helpers');

test.beforeEach(async ({ page }) => { await open(page); await expandAll(page); });

test.describe('copy', () => {
  test('typing renders into the frame', async ({ page }) => {
    await commitInput(page, 'tiHead', 'Hello world');
    await settle(page);
    expect(await page.textContent('#tHead')).toContain('Hello world');
    expect((await state(page)).text.head).toBe('Hello world');
  });

  test('Italic formats the selection without typing asterisks', async ({ page }) => {
    await commitInput(page, 'tiHead', 'plain slanted plain');
    await settle(page);
    await page.evaluate(() => {
      const el = document.getElementById('tiHead');
      el.focus();
      const range = document.createRange();
      const walker = document.createTreeWalker(el, NodeFilter.SHOW_TEXT);
      let node;
      while ((node = walker.nextNode())) {
        const i = node.nodeValue.indexOf('slanted');
        if (i >= 0) { range.setStart(node, i); range.setEnd(node, i + 7); break; }
      }
      const sel = window.getSelection();
      sel.removeAllRanges();
      sel.addRange(range);
    });
    await page.locator('#itHead').dispatchEvent('mousedown');
    await settle(page);
    expect(await page.innerHTML('#tHead')).toMatch(/<em|italic/i);
    expect(await page.textContent('#tHead')).toContain('slanted');
    expect(await page.textContent('#tHead')).not.toContain('*');
    expect(await page.locator('#tiHead').innerHTML()).toMatch(/<(em|i)>/i);
    expect(await fieldValue(page, 'tiHead')).toContain('*slanted*');
  });

  test('⌘I toggles italic on the current selection', async ({ page }) => {
    await commitInput(page, 'tiHead', 'one two three');
    await page.evaluate(() => {
      const el = document.getElementById('tiHead');
      el.focus();
      const text = [...el.childNodes].find(n => n.nodeType === 3) || el.firstChild;
      const range = document.createRange();
      range.setStart(text, 4); range.setEnd(text, 7);
      const sel = window.getSelection();
      sel.removeAllRanges(); sel.addRange(range);
    });
    const mod = process.platform === 'darwin' ? 'Meta' : 'Control';
    await page.keyboard.press(`${mod}+KeyI`);
    await settle(page);
    expect(await fieldValue(page, 'tiHead')).toMatch(/\*two\*/);
    expect(await page.locator('#tiHead').innerHTML()).toMatch(/<(em|i)>/i);
  });

  test('newlines produce line breaks', async ({ page }) => {
    await commitInput(page, 'tiHead', 'one\ntwo');
    await settle(page);
  ...
  test('an empty role collapses instead of leaving a gap', async ({ page }) => {
    await commitInput(page, 'tiEyebrow', '');
    await commitInput(page, 'tiBody', '');
    await settle(page);
  ...
  test('clearing every role hides the text block entirely', async ({ page }) => {
    for (const id of ['tiEyebrow', 'tiHead', 'tiBody']) await commitInput(page, id, '');
    await settle(page);
  ...
  test('the text switch removes the overlay without discarding the copy', async ({ page }) => {
    const head = await fieldValue(page, 'tiHead');
    ...
    await page.click('#textOn');
    await settle(page);
    expect(await fieldValue(page, 'tiHead')).toBe(head);
  ...
  test('measure stays within the grid column count', async ({ page }) => {
    await pickSeg(page, 'gCols_seg', '6');
    await settle(page);
    expect((await state(page)).text.spanCols).toBeLessThanOrEqual(6);
  });

  test('a measure narrowed by fewer columns is not lost when columns grow back', async ({ page }) => {
    await page.evaluate(() => { window.__NF.S.text.spanCols = 4; });
    await pickSeg(page, 'gCols_seg', '6');
    await settle(page);
    const narrowed = (await state(page)).text.spanCols;
    expect(narrowed).toBe(4);
    await pickSeg(page, 'gCols_seg', '12');
    await settle(page);
    expect((await state(page)).text.spanCols).toBe(narrowed);
  });

  test('measure and stack gap are exposed as designer controls', async ({ page }) => {
    expect(await page.locator('#tMeasure').count()).toBe(1);
    expect(await page.locator('#tGap').count()).toBe(1);
  });
  ...
  test('very long unbroken copy stays inside the frame', async ({ page }) => {
    await commitInput(page, 'tiHead', 'A'.repeat(400));
    await settle(page);
  ...
  test('markup characters in the copy are not injected as HTML', async ({ page }) => {
    await commitInput(page, 'tiHead', '<img src=x onerror="window.__pwned=1">');
    await settle(page);
