...
test.describe('fade mask', () => {
  ...
  test('can be locked so the fade sliders disable', async ({ page }) => {
    await page.click('#fadeLock');
    await settle(page);
    expect(await page.getAttribute('#fadeLock', 'aria-pressed')).toBe('true');
    expect(await page.locator('#maskSolid')).toBeDisabled();
    expect(await page.locator('#maskFade')).toBeDisabled();
  });
